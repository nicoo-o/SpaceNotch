...
public class PluginLoaderTests : IDisposable
{
    ...
    {
        string source = Path.Combine(AppContext.BaseDirectory, "NotchFlow.TestPlugin.dll");

        Assert.True(File.Exists(source), $"Greffon de test introuvable : {source}");

        File.Copy(source, Path.Combine(_directory, "NotchFlow.TestPlugin.dll"), overwrite: true);
    }
