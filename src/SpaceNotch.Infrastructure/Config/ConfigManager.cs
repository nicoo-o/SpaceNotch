...
/// <summary>
/// Persistance des préférences dans <c>%AppData%\SpaceNotch\config.json</c>.
///
...
public sealed class ConfigManager
{
    ...
    /// Répertoire de stockage. Par défaut <c>%AppData%\SpaceNotch</c>. Un répertoire
    ...
    public ConfigManager(string? configDirectory = null)
    {
        ...
        string folder = configDirectory ?? Path.Combine(root, "SpaceNotch");

        // Le produit s'appelait NotchFlow : ses préférences sont reprises une
        // fois, au premier lancement sous le nouveau nom. Un répertoire explicite
        // — celui des tests — n'est jamais concerné.
        if (configDirectory is null)
        {
            LegacyMigration.CopyDirectoryOnce(LegacyMigration.LegacySibling(root), folder);
        }
