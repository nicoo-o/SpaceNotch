...
Un greffon complet et fonctionnel pour NotchFlow, écrit **sans modifier l'application** et en ne
référençant qu'un seul assemblage : `NotchFlow.Core`.

Son but est de répondre à une question précise : *quelqu'un peut-il écrire un greffon en ne lisant
que cette documentation ?* Tout ce qui suit est vérifié par les tests de `tests/NotchFlow.SamplePlugin.Tests`,
...
| Élément | Valeur |
|---|---|
| Assemblage | `NotchFlow.Core` — et **rien d'autre** |
...
devient une référence à `NotchFlow.Core.dll`.
...
```bash
# 1. Construire
dotnet build samples/NotchFlow.SamplePlugin.Weather -c Release -p:Platform=x64

# 2. Installer : copier l'assemblage dans le dossier des greffons
cp samples/NotchFlow.SamplePlugin.Weather/bin/x64/Release/net10.0-windows10.0.26100.0/NotchFlow.SamplePlugin.Weather.dll \
   "$LOCALAPPDATA/NotchFlow/plugins/"

# 3. Lancer NotchFlow, puis vérifier le journal
tail -20 "$LOCALAPPDATA/NotchFlow/logs/notchflow.log"
```
...
`tests/NotchFlow.SamplePlugin.Tests` montre comment : une source de données simulée, puis le vrai
...
sur l'une d'elles, c'est le signal qu'une évolution de `NotchFlow.Core` est nécessaire.
